...
export default function Record() {
  ...
  useEffect(() => { fetch('/api/record/public').then(r => r.json()).then(setD).catch(e => setErr(String(e))) }, [])
  if (err) return <p className="empty">{err}</p>
  if (!d) return <p className="empty">Loading record…</p>
  return (<div>
    <header><h1>Record <small>every graded pick, at the price taken</small></h1><span className="meta">CLV = closing-line value: how much better than the closing price we bet, in probability points. Positive over 60+ picks is the real evidence of edge; win rate alone isn't.</span></header>
    {d.summary.length === 0 ? <p className="empty">No graded picks yet. Picks grade automatically the Tuesday after they play.</p> : <div className="tablewrap"><table style={{ minWidth: 0 }}><thead><tr><th className="l">Market</th><th>Picks</th><th>Won</th><th>Lost</th><th>Hit %</th><th>Units</th><th>Avg CLV</th><th>Avg model P</th></tr></thead>
    ...
    {d.recent.length > 0 && <><h2 style={{ marginTop: 20, fontSize: 22 }}>Recent picks</h2><div className="tablewrap"><table style={{ minWidth: 0 }}><thead><tr><th className="l">Player</th><th className="l">Game</th><th>Market</th><th>Line</th><th>Taken</th><th>Model</th><th>Close</th><th>Result</th><th>Units</th><th>CLV</th></tr></thead>
